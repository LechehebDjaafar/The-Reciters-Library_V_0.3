document.addEventListener('DOMContentLoaded', () => {
    ...
    // تعديل الة تحميل المصحف كاملاً          
        



    // دالة تحميل المصحف كاملاً
    function downloadMushaf() {
        if (!selectedReciter || !allSurahs.length) {
        ...
        const moshaf = selectedReciter.moshaf[0];
        if (!moshaf || !moshaf.server) {
            showNotification('لا توجد بيانات المصحف الكامل لهذا القارئ', 'error');
            return;
        }

        // تشغيل التحميل بشكل منفصل
        downloadCompleteMushaf();
    }

    // دالة التحميل الفعلية للمصحف
    async function downloadCompleteMushaf() {
        try {
            showNotification('جاري تجهيز المصحف للتنزيل، يرجى الانتظار...', 'info');
            
            // تعطيل الزر أثناء التحميل
            if (downloadMushafBtn) {
                downloadMushafBtn.disabled = true;
                downloadMushafBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> جاري التحميل...';
            }
            
            const zip = new JSZip();
            const mushafFolder = zip.folder(`${selectedReciter.name} - ${selectedReciter.moshaf[0].name}`);
            ...
            for (let surahIndex = 0; surahIndex < allSurahs.length; surahIndex++) {
                ...
                try {
                    // تحديث مؤشر التقدم
                    updateProgress(progressElement, surahIndex + 1, totalSurahs, surah.name);
                    
                    const response = await fetch(audioUrl);
                    if (response.ok) {
                        const audioBlob = await response.blob();
                        const fileName = `${surahId}_${surah.name.replace(/[^\w\s]/gi, '')}.mp3`;
                        mushafFolder.file(fileName, audioBlob);
                        downloadedCount++;
                        
                        showNotification(`تم تحميل: ${surah.name} (${downloadedCount}/${totalSurahs})`, 'success');
                    } else {
                        console.warn(`فشل تحميل السورة: ${surah.name}`);
                    }
                } catch (error) {
                    console.error(`خطأ في تحميل السورة ${surah.name}:`, error);
                }
                
                // إضافة تأخير قصير لتجنب الضغط على السيرفر
                await new Promise(resolve => setTimeout(resolve, 500));
            }
            
            if (downloadedCount > 0) {
                // إنشاء ملف نصي يحتوي على معلومات المصحف
                const infoText = `
    بسم الله الرحمن الرحيم

    مصحف: ${selectedReciter.name}
    الرواية: ${selectedReciter.moshaf[0].name}
    عدد السور المحملة: ${downloadedCount} من ${totalSurahs}
    ...
    بارك الله فيكم وتقبل منكم صالح الأعمال
                ...
                mushafFolder.file('معلومات_المصحف.txt', infoText);
                
                // إنشاء الملف المضغوط
                updateProgress(progressElement, totalSurahs, totalSurahs, 'جاري ضغط الملفات...');
                ...
                const zipBlob = await zip.generateAsync({
                    ...
                    updateProgress(progressElement, percent, 100, `ضغط الملفات: ${percent}%`);
                });
                ...
                link.href = url;
                link.download = `مصحف_${selectedReciter.name.replace(/[^\w\s]/gi, '')}_${selectedReciter.moshaf[0].name.replace(/[^\w\s]/gi, '')}.zip`;
                document.body.appendChild(link);
                link.click();
                document.body.removeChild(link);
                URL.revokeObjectURL(url);
                
                showNotification(`تم تحميل المصحف بنجاح! (${downloadedCount} سورة)`, 'success');
                trackUsage('download_mushaf', {
                    reciter: selectedReciter.name,
                    surah_count: downloadedCount
                });
            } else {
                showNotification('فشل في تحميل أي سورة', 'error');
            }
            ...
            console.error('خطأ في تحميل المصحف:', error);
            showNotification('حدث خطأ أثناء تحميل المصحف', 'error');
        } finally {
            // إعادة تفعيل الزر
            if (downloadMushafBtn) {
                downloadMushafBtn.disabled = false;
                downloadMushafBtn.innerHTML = '<i class="fas fa-download"></i> تحميل المصحف كاملاً';
            }
        }
    }
...
function createProgressIndicator() {
    ...
    progressBox.innerHTML = `
        <div style="margin-bottom: 1rem; color: var(--primary-color); font-size: 1.5rem;">
            <i class="fas fa-download"></i>
        </div>
        <h3 style="color: var(--text-primary); margin-bottom: 1rem;">تحميل المصحف الكريم</h3>
        ...
        <div class="progress-text" style="color: var(--text-secondary); font-size: 0.9rem;">
            جاري التحضير...
        </div>
        <div class="progress-count" style="color: var(--primary-color); font-weight: bold; margin-top: 0.5rem;">
            0 / 0
        </div>
    `;
...
function updateProgress(progressElement, current, total, status) {
